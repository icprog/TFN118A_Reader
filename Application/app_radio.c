...
//		while()
	...
	else
	{
		...
		packet[READER_UP_VERSION_IDX] |= TAG_HDVER_NUM << TAG_HDVERSION_POS;//硬件版本号
		packet[READER_UP_VERSION_IDX] |= TAG_SFVER_NUM << TAG_SFVERSION_POS;//软件版本号
		packet[READER_UP_STYPE_IDX] = READER_UP_SENSORTYPE_SchoolWatch;//类型
		packet[TAG_SDATA_IDX] |= MSG_Store.MSG_Seq << TAG_MSEQ_Pos;//传感数据
		cmd_packet.length = READER_UP_SDATA_IDX;//PAYLOAD长度 
